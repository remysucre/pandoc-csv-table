--[[
csv-table.lua — Pandoc Lua filter: fenced-div CSV → pandoc.Table
Requires pandoc ≥ 2.17 (Lua 5.4, pandoc.Cell/Row/TableHead/TableFoot API).
...
Or inline, no code fence (simple data without commas in values):
...
--------------
  delimiter / delim / sep
      ...
      Tip: pandoc's Markdown reader expands literal tab characters to spaces
      before the filter runs, so delimiter="\t" (TSV) only works when pandoc
      is fed a pre-parsed AST or a non-Markdown format.  For visual column
      alignment in Markdown source, use "|" as the delimiter instead.

  quote
      Quotation character for fields containing the delimiter or newlines.
      Default: '"'.  Use "none" to disable quoting entirely.

  escape
      Escape character inside quoted fields.  Default: nil (doubled-quote
      escaping is used instead, i.e. "" → ").

  keep-space / keepspace
      "true" to preserve whitespace immediately following the delimiter.
      Default: false (leading spaces after delimiter are stripped).

  header
      "false" to treat every row as a data row (no header).
      Default: first row becomes the TableHead.

  caption / title
      Plain text (or inline Markdown) for the table caption.

  align / aligns
      Comma-separated per-column alignment flags:
        l / left    → AlignLeft
        r / right   → AlignRight
        c / center  → AlignCenter
        d / default → AlignDefault  (the default for unspecified columns)
      Example:  align="l,r,c,d"

CSV PARSING
-----------
The LPeg grammar mirrors Text.Pandoc.CSV (Haskell):
  • Quoted cells may span multiple lines.
  • Inside a quoted cell, the escape character (if set) prefixes a literal
    character; otherwise consecutive quote chars collapse to one.
  • Blank lines (a line with no delimiter and no content) terminate the
    row sequence, matching Parsec's sepEndBy behaviour for pCSVRow.
